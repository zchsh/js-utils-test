...
Personally I think of layouts as sets of "strings" - each string renders a headline, a paragraph, a label, and so on. I find the mnemonic `An orphan is alone from the beginning; a widow is alone at the end` to be helpful. With these interpretations, a `widow` is a short word or word group that is `alone at the end` of a string or layout element<sup id="a2">[2](#f2)</sup>. These functions replace spaces near the `end` of a string. So, it felt intuitive to use the term `window`.
